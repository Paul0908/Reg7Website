import {
  Injectable, OnDestroy, Renderer2, RendererFactory2,
} from '@angular/core';
import {
  BehaviorSubject, fromEventPattern, Observable, Subject, Subscription,
} from 'rxjs';
...
export default class ScrollDetectionService implements OnDestroy {
  ...
  private createOnScrollObservable(renderer: Renderer2) {
    ...
    this.onScroll$ = fromEventPattern<Event>(createScrollEventListener, () => removeScrollEventListener()).pipe(takeUntil(this._destroy$));
  ...
  subscribeToOnScrollAndSetScrollHeight(){
    this.onScrollSubscription = this.onScroll$.subscribe((scrollEvent) => {
      ...
      const scrollHeight = (scrollPath[1] as any).scrollY;
